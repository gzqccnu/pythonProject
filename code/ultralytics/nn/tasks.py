...
# from ultralytics.nn.modules.block import  import
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    """
    import ast
    from ultralytics.nn.modules.block import MobileViTBlock, MV2Block
    # Args
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        #globals().update({'MV2Block': MV2Block})
        #globals().update({'MobileViTBlock': MobileViTBlock})
        if isinstance(m, str):
            if "nn." in m:
                m = getattr(torch.nn, m[3:])
            elif m in globals():
                m = globals()[m]  # get module from globals
            else:
                # 尝试从自定义模块导入
                if m == 'MV2Block':
                    m = MV2Block
                elif m == 'MobileViTBlock':
                    m = MobileViTBlock
                else:
                    raise KeyError(f"Module '{m}' not found")
        ...
        if m in base_modules:
            ...
            if m is C3k2:  # for M/L/X sizes
                ...
                if scale in "mlx":
                    args[3] = True
            # if m is C3k2:
            #     # 默认参数：c3k=False, e=0.5, g=1, shortcut=True
            #     default_args = [False, 0.5, 1, True]
            #     # 合并用户参数和默认参数
            #     user_args = args[1:]  # 用户提供的参数（如 [False]）
            #     full_args = [c1, c2, n] + user_args + default_args[len(user_args):]
            #     args = full_args
            # else:
            #     args = [c1, c2, *args[1:]]
            #     if m in repeat_modules:
        ...
        #m_ = torch.nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args[2:])  # 跳过前两个参数
